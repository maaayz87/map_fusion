...
def ICP(src_cloud, tar_cloud, max_iter, tolerance):
...
def icpPointsPick(pc1, pc2, gps1, gps2, pair_num, radius):
    ...
    #排序取需要的前n对索引
    closest_points_sorted = sorted(closest_points, key=lambda x: x[2])[:pair_num]
...
if __name__ == '__main__':
    ...
    #获取建图得到的原始odom数据
    origin_odom1 = gfx.odomfixer(odom1_path, len(origin_gps1))
    origin_odom2 = gfx.odomfixer(odom2_path, len(origin_gps2))
    origin_odom2[:, 0] = -origin_odom2[:, 0]
    X = origin_odom2[:,0]
    Y = origin_odom2[:,1]
    ...
    #获取地图对齐到gps坐标的旋转
    R1 = gfx.pathsAlign(origin_odom1, origin_gps1)
    R2 = gfx.pathsAlign(origin_odom2, origin_gps2)
    ...
    R = np.array([
      [0.999404, 0.0331256, -0.00970038],
      [-0.0330379, 0.999413, 0.00907391],
      [0.00999527, -0.00874802, 0.999912]
    ])
    t = np.array([3.98728, 8.80403, -2.81544])
    #将变换应用到第二张地图
    pc2_array = (R @ pc2_array.T).T + t
    print("STD Trans done")
    ################
    ...
    pc1_ds = pclpy.octree_voxel_downsample(pc1_fixed, 1)
    pc2_ds = pclpy.octree_voxel_downsample(pc2_fixed, 1)
    ...
    pcl.io.savePCDFileASCII(ICP_save_path, color_pc)
    print("ICP map Saved")
